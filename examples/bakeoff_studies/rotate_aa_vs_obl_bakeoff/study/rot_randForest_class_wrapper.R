...
rot_randForest_class_wrapper = function(
    Y_train,
    X_train_df,
    X_test_df,
    num_rota = 200
  ...
  timing = rep(NA, times = 1)
    ...
    yhat_train_samples[1, ] = as.integer(predict(fit, rotated_X_train)) - 1
    yhat_test_samples[1, ] = as.integer(predict(fit, rotated_X_test)) - 1

...
rot_extra_trees_reg_wrapper = function(
    Y_train,
    Y_test,
    X_train_df,
    X_test_df,
  ...
  timing = rep(NA)
    ...
    yhat_train_samples[1, ] = predict(fit, rotated_X_train)$predictions
    yhat_test_samples[1, ] = predict(fit, rotated_X_test)$predictions

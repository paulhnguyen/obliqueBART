...
)
{
  ...
  for (chain in 1:n_chains){
    ...
    train_time = system.time(
      fit <- BART::pbart(
        y.train = as.integer(Y_train),
        x.train = rotated_X_train, 
        x.test = rotated_X_test,
    ...
    phat_train_samples[start_index:end_index, ] = pnorm(fit$yhat.train)
    phat_test_samples[start_index:end_index, ] = pnorm(fit$yhat.test)

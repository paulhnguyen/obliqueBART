...
# Details: spectf heart dataset (https://archive.ics.uci.edu/dataset/96/spectf+heart
# Dependencies: 
set.seed(2)
data_dir_train = "../source/SPECTF.train"
data_dir_test = "../source/SPECTF.test"
save_dir = "../data/"
name = "spectf_heart"

# setup -------------------------------------------------------------------
data_train = read.table(data_dir_train, sep = ",", header = FALSE, fill = TRUE)
data_test = read.table(data_dir_test, sep = ",", header = FALSE, fill = TRUE)
...
Y = as.integer(ifelse(data[,1] == "1", 1, 0))
X_cont = data[,c(2:45)]

...
# Details: climate model crashes dataset (https://archive.ics.uci.edu/dataset/252/climate+model+simulation+crashes)
# Dependencies: 
set.seed(2)
data_dir = "../source/pop_failures.dat"
save_dir = "../data/"
name = "climate_model_crashes"

# setup -------------------------------------------------------------------
data = read.table("pop_failures.dat", sep="", header = T)
...
Y = as.integer(ifelse(data[,21] == 1, 1, 0))
X_cont = data[,3:20]

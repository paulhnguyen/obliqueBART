...
# Details: acute inflammation 2 dataset (https://archive.ics.uci.edu/dataset/184/acute+inflammations)
# Dependencies: 
set.seed(2)
data_dir = "../source/inflammations_diagnosis.data"
save_dir = "../data/"
name = "acute_inflammations_2"

# setup -------------------------------------------------------------------
data = read.table("inflammations_diagnosis.data", fileEncoding="UTF-16", dec=",")
...
Y = as.integer(ifelse(data[,8] == "yes", 1, 0))
X_cont = data[, c(1)] 
X_cont <- sapply( X_cont, as.numeric )
X_cat = data[, c(2:6)] #put  matrix(0) for no categorical variables

                       ...
                       std::vector<double> &x)
{
  ...
  for(int i = 0; i < num_constraints; ++i){
    ...
    Rcpp::Rcout << "phi_x: " << phi_x << std::endl;
    Rcpp::Rcout << "b_i: " << b_vec[i] << std::endl;
    if(phi_x >= b_vec[i]){
      results = false;
      break;
    }
...
void lin_ess(std::vector<double> &sampled_point,
             ...
             RNG &gen)
{
  if(!check_constraints(phi_vec, b_vec, init_point)){
    Rcpp::Rcout << "[lin_ess]: supplied inital point outside polytope!" << std::endl;
    Rcpp::Rcout << "  The constraints of child polytope are:" << std::endl;
    print_constraints(phi_vec, b_vec);
    Rcpp::Rcout << " The putative point in the polytope is :";
    for(std::vector<double>::iterator x0_it = init_point.begin(); x0_it != init_point.end(); ++x0_it) Rcpp::Rcout << " " << *x0_it;
    Rcpp::Rcout << std::endl;
    Rcpp::stop("[lin_ess]: supplied inital point outside polytope!");
  }
  ...
  for(int j = 0;  j < p; ++j) v_vec.push_back(gen.normal() / (2 * d));
                    ...
                    RNG &gen)
{
  ...
  if(!check_constraints(child_phi_vec, child_c_vec, x0)){
    ...
    int max_init_samples = 100;
    for(int rep = 0; rep < max_init_samples; ++rep){
      lin_ess(tmp_sample, parent_phi_vec, parent_c_vec, x0, d, p, gen);
      ...
      if(tmp_x0.size() != 0){
        ...
        while(!in_child && counter < 20){
          ...
          for(int rep = 0; rep < 50; ++rep){
            lin_ess(tmp_sample, child_phi_vec, tmp_c, tmp_x0, d, p ,gen);
        ...
        Rcpp::Rcout << "Could not find a point in the initial shifted polytope" << std::endl;
        ...
        Rcpp::stop("could not find point in initial shifted polytope");
      }
    }
...
void get_child_analytic_center(std::map<int, std::vector<double>> &analytic_centers,
                               ...
                               RNG &gen)
{
  // we have accepted a grow proposal and need to find the child's analytic center

  // step 1: find a point inside the child polytope
  std::vector<double> feasible_pt;
  int found_point = 0;

  // start by checking if the parent analytic center is in the child polytope
  std::vector<double> parent_center;

  // check to make sure we have the parent analytic center
  if (analytic_centers.find(nx_nid) == analytic_centers.end()){
    ...
    parent_center = parent_center_it->second;
  }

  // since we are trying to be very careful, double check the parent analytic center is feasible for the parent
  ...
  // if this gets hit, something has gone very wrong
  if (!check_constraints(parent_phi_vec, parent_c_vec, parent_center)){
    Rcpp::Rcout << "[get_child_analytic_center]: parent analytic center does not satisfy the constraints! (node " << nx_nid << ")." << std::endl;
    t.print();
    Rcpp::Rcout << "  The constraints of the polytope are:" << std::endl;
    print_constraints(parent_phi_vec, parent_c_vec);
    Rcpp::Rcout << " The putative point in the polytope is :";
    for(std::vector<double>::iterator x0_it = parent_center.begin(); x0_it != parent_center.end(); ++x0_it) Rcpp::Rcout << " " << *x0_it;
    Rcpp::Rcout << std::endl;
    Rcpp::stop("[get_child_analytic_center]: parent analytic center does not satisfy the constraints!");
  }

  // check if parent center is feasible for child
  ...
  if (check_constraints(child_phi_vec, child_c_vec, parent_center)){
    feasible_pt = parent_center;
    found_point = 1;
  }

  // if the parent center does not satisfy the constraints of the child,
  if (found_point == 0){
    // see if there is a point in our dataset that strictly satisfies the child's constraints
    std::vector<double> feasible_obs;
    ...
    if(ss_it->second.size() > 0){
      ...
      for (int i = 0; i < ss_it->second.size(); ++i){ // i indexes the observations in the leaf
        feasible_obs.clear();
        int j = ss_it->second[i]; // j is the observation number in the full dataset
        for (int k = 0; k < di.p_cont; ++k) feasible_obs.push_back(di.x_cont[k + j * di.p_cont]); // k indexes the continuous covariates
        if (check_constraints(child_phi_vec, child_c_vec, feasible_obs)){
          // we found a point!
          feasible_pt = feasible_obs;
          found_point = 1;
          break;
        }
      } // closes for loop over points in leaf
    } // closes check to see if there are any observations falling into the leaf
  }

  // if no observations strictly satisfy the constraints, try infeasible start analytic center
  if (found_point == 0){
    std::vector<double> inf_ac_pt;

    // this should rarely hit in our simulation studies, so print out a warning if it does
    // remove this message when package is published
    Rcpp::Rcout << "[get_child_analytic_center]: trying infeasible start analtyic center." << std::endl;

    // recreate our constraints in the form Ax < b
    // where A is an arma::mat and x & b are arma::vec
    int n = child_c_vec.size(); // number of constraints
    arma::mat A(n, di.p_cont, arma::fill::zeros);
    for (int i = 0; i < n; ++i){
      std::map<int, double> tmp_map = child_phi_vec[i];
      for (std::map<int, double>::iterator it = tmp_map.begin(); it != tmp_map.end(); ++it) A(i, it->first) = it->second;
    }
    arma::vec b(n, arma::fill::zeros);
    for (int i = 0; i < n; ++i) b[i] = child_c_vec[i];
    arma::vec x(di.p_cont, arma::fill::zeros);
    for (int i = 0; i < di.p_cont; ++i) x[i] = parent_center[i];

    // run infeasible start analytic center
    inf_analytic_center(x, A, b);
    for (int i = 0; i < di.p_cont; ++i) inf_ac_pt.push_back(x[i]);

    // check to see if we returned a feasible point
    if (check_constraints(child_phi_vec, child_c_vec, inf_ac_pt)){
      // we found a point!
      feasible_pt = inf_ac_pt;
      found_point = 1;
    } else {
      // not necessary, but may help with debugging
      // remove this message when package is published
      Rcpp::Rcout << "[get_child_analytic_center]: failed to find feasible point with infeasible start analtyic center." << std::endl;
    }
  }

  // NOTE: something is wrong with nested sampling
  // when we hit this there is a fatal error

  // if all else fails, try nested sampling scheme
  // if (found_point == 0){
  //   std::vector<double> nested_pt;

  //   // this should rarely hit in our simulation studies, so print out a warning if it does
  //   // remove this message when package is published
  //   Rcpp::Rcout << "[get_child_analytic_center]: trying nested sampling scheme." << std::endl;

  //   int d = t.get_ptr(child_nid)->get_depth();
  //   get_init_point(nested_pt, parent_phi_vec, parent_c_vec, child_phi_vec, child_c_vec, d, di.p_cont, gen);

  //   // check to see if we returned a feasible point
  //   if (check_constraints(child_phi_vec, child_c_vec, nested_pt)){
  //     // we found a point!
  //     feasible_pt = nested_pt;
  //     found_point = 1;
  //   } else {
  //     // not necessary, but may help with debugging
  //     // remove this message when package is published
  //     Rcpp::Rcout << "[get_child_analytic_center]: failed to find feasible point with nested sampling scheme." << std::endl;
  //   }
  // }

  // final check to see if we should continue
  if (found_point == 0){
    Rcpp::Rcout << "[get_child_analytic_center]: we have exhausted all options, but could not find a feasible point." << std::endl;
    Rcpp::stop("[get_child_analytic_center]: failed to find a feasible point to start analytic center calculations :(");
  } else {
    // check one more time that the point is feasible
    if (!check_constraints(child_phi_vec, child_c_vec, feasible_pt)){
      // if this gets hit, go back and put this same error message under each method to find out what is broken
      Rcpp::Rcout << "[get_child_analytic_center]: something has gone very wrong." << std::endl;
      Rcpp::Rcout << "One of our methods thinks it found a feasible point when it really did not." << std::endl;
      Rcpp::Rcout << "We are trying to find a feasible point for node " << child_nid << "." << std::endl;
      t.print();
      Rcpp::Rcout << "  The constraints of the polytope are:" << std::endl;
      print_constraints(parent_phi_vec, parent_c_vec);
      Rcpp::Rcout << " The putative point in the polytope is :";
      for(std::vector<double>::iterator x0_it = feasible_pt.begin(); x0_it != feasible_pt.end(); ++x0_it) Rcpp::Rcout << " " << *x0_it;
      Rcpp::Rcout << std::endl;
      Rcpp::stop("[get_child_analytic_center]: we made a big mistake!");
    }
  }

  // if we've made it this far, we have a feasible point to start the feasible-start analytic center method

  // step 2: use point to initialize feasible-start analytic center method
  std::vector<double> child_center = feasible_pt;
  calc_analytic_center(child_center, child_phi_vec, child_c_vec, di.p_cont);

  // check that the calculated child analytic center is feasible
  if(!check_constraints(child_phi_vec, child_c_vec, child_center)){
    Rcpp::Rcout << "[get_child_analytic_center]: calc_analytic_center returned infeasible point for node " << child_nid << "." << std::endl;
    t.print();
    Rcpp::Rcout << "  The constraints of the polytope are:" << std::endl;
    print_constraints(parent_phi_vec, parent_c_vec);
    Rcpp::Rcout << " The putative point in the polytope is :";
    ...
    Rcpp::Rcout << std::endl;
    Rcpp::stop("[get_child_analytic_center]: calc_analytic_center failed to return feasible point!");
  }

  // step 3: update analytic_centers
...
{   
    ...
    if (A.n_rows != n) Rcpp::Rcout << "[inf_analytic_center]: A has " << A.n_rows << " rows but b has " << n << " elements!" << std::endl;
    if (A.n_cols != d) Rcpp::Rcout << "[inf_analytic_center]: A has " << A.n_cols << " cols but x has " << d << " elements!" << std::endl;
    ...
    while (iter < max_it){
        ...
        // stopping conditions (original)
        // if (arma::norm(y + A * x - b, 2) < epsilon && arma::norm(r, 2) < epsilon) break;

        // relax stopping conditions
        // stop if we find a feasible point
        // then we can use this point to initialize the feasible start analytic center which is more stable
        if (arma::norm(A * x - b, 2) < 0) break;

        dx = arma::solve((A.t() * H * A), (A.t() * g - A.t() * H * rp));
        dy = -A * dx - rp;
        dv = -H * dy - g - v;
        ...
        //     Rcpp::Rcout << "A: ";
        //     for (arma::mat::iterator it = A.begin(); it != A.end(); ++it) Rcpp::Rcout << (*it) << ", ";
        //     Rcpp::Rcout << std::endl;
        //     Rcpp::Rcout << "b: ";
        ...
        //     Rcpp::Rcout << std::endl;
        //     Rcpp::stop("Unable to find a feasible step size in 100 iterations.");
        // }
        // else it = 0;
        it = 0;
        ...
        //     for (int i = 0; i < d; ++i) Rcpp::Rcout << x[i] << ", ";
        //     Rcpp::Rcout << std::endl;
        //     Rcpp::stop("Unable to find an acceptable step size in 100 iterations.");
        // }
        // else it = 0;
        it = 0;
...
void get_child_analytic_centers(std::map<int, std::vector<double>> &analytic_centers,
                                int &nx_nid,tree &t, int &p_cont, RNG &gen)
{
  tree::tree_p nx = t.get_ptr(nx_nid);
  
  // get all continuous constraints for the parent
  std::vector<std::map<int,double>> parent_phi_vec;
  std::vector<double> parent_c_vec;
  std::vector<double> parent_aa_lower(p_cont, -1.0);
  std::vector<double> parent_aa_upper(p_cont, 1.0);
  nx->get_cont_constraints(parent_phi_vec, parent_c_vec, parent_aa_lower, parent_aa_upper, p_cont);
  
  if(analytic_centers.find(nx_nid) == analytic_centers.end()){
    Rcpp::Rcout << "[get_child_analytic_centers]: Trying to find analytic center for node " << nx_nid;
    Rcpp::Rcout << "Only have analytic centers for nodes:";
    for(std::map<int,std::vector<double>>::iterator it = analytic_centers.begin(); it != analytic_centers.end(); ++it) Rcpp::Rcout << " " << it->first;
    Rcpp::Rcout << std::endl;
    Rcpp::stop("Could not find analytic center for requested parent!");
  } else{
    for(int child_nid = 2*nx_nid; child_nid <= 2*nx_nid+1; ++child_nid){
      std::vector<std::map<int, double>> child_phi_vec;
      std::vector<double> child_c_vec;
      std::vector<double> child_aa_lower(p_cont, -1.0);
      std::vector<double> child_aa_upper(p_cont, 1.0);
      tree::tree_p child_nx = t.get_ptr(child_nid);
      
      child_nx->get_cont_constraints(child_phi_vec, child_c_vec, child_aa_lower, child_aa_upper, p_cont);
      
      // now get the analytic center of the parent
      std::map<int,std::vector<double>>::iterator parent_center_it = analytic_centers.find(nx_nid);
      std::vector<double> child_center = parent_center_it->second;
      // find a point in the child polytope
      int d = t.get_ptr(child_nid)->get_depth();
      get_init_point(child_center, parent_phi_vec, parent_c_vec, child_phi_vec, child_c_vec, d, p_cont, gen);
      // at this point, child_center is just a point in the child polytope
      // sanity check:
      if(!check_constraints(child_phi_vec, child_c_vec, child_center)){
        Rcpp::Rcout << "[get_child_analytic_centers]: Currently trying to find analytic center of new child " << child_nid << std::endl;
        t.print();
        Rcpp::Rcout << "  The constraints of child polytope are:" << std::endl;
        print_constraints(child_phi_vec, child_c_vec);
        Rcpp::Rcout << " The putative point in the polytope is :";
        ...
        Rcpp::Rcout << "  Here are the analytic centers that were found:" << std::endl;
        for(std::map<int, std::vector<double>>::iterator it = analytic_centers.begin(); it != analytic_centers.end(); ++it){
          Rcpp::Rcout << " Node " << it->first << ":";
          for(std::vector<double>::iterator x_it = it->second.begin(); x_it != it->second.end(); ++x_it) Rcpp::Rcout << " " << *x_it;
          Rcpp::Rcout << std::endl;
        }
        
        Rcpp::stop("Putative center is not in polytope!");
      }
      calc_analytic_center(child_center, child_phi_vec, child_c_vec, p_cont);
      // now at this point, child_center contains analytic center of child polytope
      analytic_centers.insert(std::pair<int, std::vector<double>>(child_nid, child_center));
    } // closes for loop over left and right child node ids
  } // closes if/else checking that we have computed analytic center of parent node.
}

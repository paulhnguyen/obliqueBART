    ...
    for(int_it it = nx_it->second.begin(); it != nx_it->second.end(); ++it){
      ...
      if(rule.is_cat){
        ...
        if(tmp_x < rule.c) nxl_it->second.push_back(i);
          ...
          Rcpp::Rcout << "[update_tree] i = " << i << " phi'x = " << tmp_x << " c = " << rule.c << std::endl;
          Rcpp::Rcout << "Phi: ";
          for(auto it = rule.phi.cbegin(); it != rule.phi.cend(); ++it)
        {
            Rcpp::Rcout << it->first << ": " << it->second <<  "\n";
        }
          Rcpp::stop("[update_tree/compute_ss_grow]: could not assign observation to left or right child in oblique split!");
        }
...
void grow_tree(tree &t, std::map<int, std::vector<double>> &analytic_centers, suff_stat &ss_train, suff_stat &ss_test, int &accept, rule_diag_t &rule_diag, double &sigma, data_info &di_train, data_info &di_test, tree_prior_info &tree_pi, RNG &gen)
{
  ...
  draw_rule(rule, t, nx_nid, tmp_x0, di_train, tree_pi, gen); // draw the rule.
...
void prune_tree(tree &t, std::map<int, std::vector<double>> &analytic_centers, suff_stat &ss_train, suff_stat &ss_test, int &accept, double &sigma, data_info &di_train, data_info &di_test, tree_prior_info &tree_pi, RNG &gen)
{
...
void update_tree(tree &t, std::map<int, std::vector<double>> &analytic_centers, suff_stat &ss_train, suff_stat &ss_test, int &accept, rule_diag_t &rule_diag, double &sigma, data_info &di_train, data_info &di_test, tree_prior_info &tree_pi, RNG &gen)
{
  ...
  if(gen.uniform() < PBx) grow_tree(t, analytic_centers, ss_train, ss_test, accept, rule_diag, sigma, di_train, di_test, tree_pi, gen);
  else prune_tree(t, analytic_centers, ss_train, ss_test, accept, sigma, di_train, di_test, tree_pi, gen);

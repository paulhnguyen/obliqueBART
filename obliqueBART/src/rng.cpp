...
std::vector<double> RNG::pick_direction(int dim) { 
    std::vector<double> dir(dim); // Create a vector of length dim
    std::vector<double> dir_standardized(dim); // Create a vector of length dim
    double temp_s = 0.0;

    // Generate and populate the vector with values
    for (int i = 0; i < dim; ++i) {
        // Generate a random direction for each element in the vector
        double direction = normal(0.0, 1.0);
        dir[i] = direction;
        temp_s += pow(direction, 2.0);
    } 

    // standardizing direction vector
    double magnitude = sqrt(temp_s);
    for (int i = 0; i < dim; ++i)
    {
        dir_standardized[i] = dir[i] / magnitude;
    }
    

    return dir_standardized; // Return the standardized vector
}

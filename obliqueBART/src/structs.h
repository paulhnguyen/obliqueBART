...
public:
  ...
  double a_theta; // alpha parameter in prior on theta for selecting oblique-splits
  double b_theta; // beta parameter in prior on theta for selecting oblique-splits
  ...
  // counters for adaptive splits (2 sparse)
  std::vector<double>* theta_combination; // A vector that holds the probabilities that the combination will be used in a split
  std::vector<int> *combination_count; // A vector that holds the number of times that the combination of 2 variables is used in a split 

  // stuff for adaptive splits dp
  int alpha_dp;
  ...
  // spike slab stuff
  double spike_slab_theta;
  std::pair<int, int> *entry_counts; // counts the number of non-zero entries for phi. the first of the pair will represent the non-zero entries, nad end second of the pair represents the number of zero-entries in phi (over the entire tree)
  ...
  tree_prior_info(){
    ...
    aa_count = 0; // 0 pointer
    obl_count = 0; // 0 pointer

...
Conditionally on $\theta,$ we draw $p$ binary indicators $\gamma_{1}, \ldots, \gamma_{p} \vert \theta \overset{i.i.d.}{\sim} \textrm{Bernoulli}(\theta);$ (ii) for each $j = 1, \ldots, p_{cont},$ we draw $\phi_{j} \sim \textrm{Normal}({0},{1})$ if $\gamma_{j} = 1$ and $\phi_{j} = 0$ otherwise; and (iii) finally, we re-scale $\phi$ to have unit norm. 
